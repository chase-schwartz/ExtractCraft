...
public class BaseStashMenu extends AbstractContainerMenu {
    ...
    private static final int SORT_OFFSET = 400_000;
    ...
    public boolean clickMenuButton(Player player, int id) {
        ...
        boolean changed;
        if (id >= MOVE_BASE_TO_BASE_OFFSET) {
    ...
    private boolean moveStashToBase(ServerPlayer player, int stashDisplayIndex, RaidEquipmentSlot target) {
        ...
        RaidInventory.AddResult result = switch (target) {
            case PRIMARY_WEAPON, SECONDARY_WEAPON -> candidate.addToWeaponSlot(PlayerStashService.copyItem(item), target);
            case BACKPACK -> candidate.addToBackpack(PlayerStashService.copyItem(item));
            case VEST -> candidate.addToVest(PlayerStashService.copyItem(item), profile);
            case SAFE_BOX -> candidate.addToSafeBox(PlayerStashService.copyItem(item), profile);
        };
    ...
    private boolean moveBaseToBase(ServerPlayer player, RaidEquipmentSlot source, int sourceIndex, RaidEquipmentSlot target) {
        ...
        RaidInventory.AddResult result = candidate.move(source, sourceIndex, target, profile);
        if (result.movedCount() < item.count()) {
            player.sendSystemMessage(Component.literal(result.success() ? "Base target does not have enough capacity." : result.message()));
            return false;
        }
    ...
    private void rebuildDisplays() {
        ...
        if (baseInventory.primaryWeapon() != null) {
            baseDisplay.setItem(PRIMARY_WEAPON_START, displayStack(baseInventory.primaryWeapon()));
        }
        if (baseInventory.secondaryWeapon() != null) {
            baseDisplay.setItem(SECONDARY_WEAPON_START, displayStack(baseInventory.secondaryWeapon()));
        }
        fillDisplay(baseDisplay, BACKPACK_START, BACKPACK_DISPLAY_SLOTS, baseInventory.backpack().items());
        fillDisplay(baseDisplay, VEST_START, VEST_DISPLAY_SLOTS, baseInventory.vest().items());
        fillDisplay(baseDisplay, SAFE_BOX_START, SAFE_BOX_DISPLAY_SLOTS, baseInventory.safeBox().items());
        ...
        for (int displayIndex = 0; displayIndex < Math.min(stashCapacity, stashItems.size()); displayIndex++) {
            ...
            stashDisplay.setItem(displayIndex, displayStack(indexedItem.item()));
        }
    }

    private void fillDisplay(SimpleContainer container, int start, int maxSlots, List<RaidInventoryItem> items) {
        for (int i = 0; i < Math.min(maxSlots, items.size()); i++) {
            container.setItem(start + i, displayStack(items.get(i)));
        }
    }
